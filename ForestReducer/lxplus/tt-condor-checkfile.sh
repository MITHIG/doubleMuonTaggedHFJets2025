...
if [[ $# -ne 16 ]]; then
    echo "usage: ./tt-condor-checkfile.sh [executable file] [input dir] [output dir] [max jobs] [log dir] [IsData] [MinJetPT] [IsPP] [UseHybrid] [IsDebug] [UseTrackVtxInfo] [Fraction] [MuTrackMatchDRCut] [GenRecoMuonMatchDRCut] [applyConstituentMatching] [PFJetCollection]"
    exit 1
fi
...
Arguments    = $EXEFILE $inputname $DEST_CONDOR ${outputfile} $CMSSW_VERSION $IsData $MinJetPT $IsPP $USEHYBRID $IsDebug $UseTrackVtxInfo $Fraction $MuTrackMatchDRCut $GenRecoMuonMatchDRCut $applyConstituentMatching $PFJetCollection
