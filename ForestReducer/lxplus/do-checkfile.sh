...
OUTPUTPRIDIR="/eos/cms/store/group/phys_heavyions/"$USER"/skim_condor"
...
do
    ...
    if [ "$submit_jobs" -eq 1 ] ; then
        set -x
        ./tt-condor-checkfile.sh $EXEFILE "$INPUTFILELIST" $OUTPUTDIR $MAXFILENO $LOGDIR $IsData $MinJetPT $IsPP $useHybrid $IsDebug $UseTrackVtxInfo $Fraction $MuTrackMatchDRCut $GenRecoMuonMatchDRCut $applyConstituentMatching $PFJetCollection
        set +x
    fi
